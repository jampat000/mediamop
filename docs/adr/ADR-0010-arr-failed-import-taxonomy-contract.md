# ADR-0010: *arr* failed-import taxonomy and operator contract

## Status

Accepted — documents behavior implemented in `mediamop.modules.arr_failed_import` and queue attention / cleanup drives in the product shell.

**Failed-imports (taxonomy + policy contract):** **Complete — subject to live prod testing/signoff** on real queue wording and *arr versions only. Internal contract questions are settled here; upstream queue execution scope is ADR-0011 (same completion framing).
...
`PENDING_WAITING` and `UNKNOWN` map to **no** policy key (`cleanup_policy_key_for_outcome` returns `None`). Operators cannot assign remove/blocklist to them; cleanup automation does not treat them as cleanup targets.

### Visible settings rows

The **Failed imports** operator UI exposes **six** rows per axis (TV / movies), one per `FailedImportCleanupPolicyKey` / handling field. That list is **exclusive and complete** for policy-backed classes. It is **not** a row per enum member: two enum members deliberately have no row.
