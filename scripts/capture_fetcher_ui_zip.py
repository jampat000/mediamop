"""One-off: boot API + Vite preview (isolated home), bootstrap+login, screenshot every Fetcher tab, write a zip.

Requires: Chromium for Playwright (``python -m playwright install chromium`` from apps/backend venv),
``npm run build`` in ``apps/web`` (``dist/``), and ``alembic`` on PATH or use the backend venv Python.

Optional: ``MEDIAMOP_SESSION_SECRET`` — if unset, a random secret is used for this run only.

Output: ``<repo>/artifacts/fetcher-ui-tabs.zip`` (PNG files inside).
"""
...
ZIP_PATH = ARTIFACTS_DIR / "fetcher-ui-tabs.zip"

BOOTSTRAP_USER = "capture-fetcher-ui"
BOOTSTRAP_PASS = "capture-fetch-pass-min8"
...
def _e2e_home() -> str:
    ...
    return str(Path(tempfile.mkdtemp(prefix="mediamop_capture_fetcher_")))
...
def _capture_zip() -> Path:
    ...
    shot_dir = Path(tempfile.mkdtemp(prefix="fetcher_shots_"))
    try:
        ...
        tabs: list[tuple[str, str]] = [
            ...
            ("03-tv-sonarr", "Sonarr (TV)"),
            ("04-movies-radarr", "Radarr (Movies)"),
            ("05-failed-imports", "Failed imports"),
        ]
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                page.goto(f"{web_origin}/app/fetcher", wait_until="domcontentloaded")
                page.wait_for_timeout(800)

                tab_nav = page.get_by_test_id("fetcher-section-tabs")
