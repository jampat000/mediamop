"""Singleton row for Fetcher-owned Sonarr/Radarr search preferences and optional in-app connection fields."""
...
class FetcherArrOperatorSettingsRow(Base):
    """One row (``id = 1``) — missing/upgrade search lanes for TV (Sonarr) and movies (Radarr)."""

    __tablename__ = "fetcher_arr_operator_settings"
    __table_args__ = (CheckConstraint("id = 1", name="ck_fetcher_arr_operator_settings_singleton"),)
